...
__all__ = ['EnvTyper', 'app', 'migrate', 'DBT_BASE_ARGS', 'build_tables', 'ingest', 'ingest_538', 'resimulate',
           'backtest']
...
def migrate(interactive: bool = True):
    ...
    typer.secho('Adding seed data with dbt', fg=typer.colors.BLUE)
    _ = dbt.main.handle_and_check(['seed'] + DBT_BASE_ARGS)
...
def build_tables(args: typing.List[str] = typer.Option([], help='Additional arguments passed to `dbt run`')):
    ...
    _ = dbt.main.handle_and_check(['run'] + DBT_BASE_ARGS + list(args))
